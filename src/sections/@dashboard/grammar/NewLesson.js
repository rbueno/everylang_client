...
export default function BusinessEdit({ editingWorkspace, lessonId, isEdit }) {
  ...
  const handleAdGenerator = async () => {
    ...
    const payload = {...data, languageLevel: level, learningLanguage: language, lessonType: 'grammar'}
    try {
      ...
      setSubmitting(false)
      // redirect
      push(PATH_DASHBOARD.lessonGrammar.content(data.lesson._id));
  ...
  const handleUpdateLesson = async () => {
    ...
    const payload = {...data, lessonId, languageLevel: level, learningLanguage: language, lessonType: 'grammar'}
    try {
      ...
      push(PATH_DASHBOARD.lessonGrammar.content(lessonId));
    } catch (error) {
      enqueueSnackbar(error.message && error.message, { variant: 'error' });
      console.error(error);
    }
